...
class Vamtam_Elementor_Utils {
	...
	public static function is_wc_mod_active( $wc_mod_name = '', $from_db = false ) {
		...
		$theme_setting_prefix = 'vamtam_theme_';
		if ( ! empty( \Vamtam_Elementor_Utils::get_theme_site_settings( "{$theme_setting_prefix}enable_all_wc_mods", $from_db ) ) ) {
			// All theme wc mods enabled by user pref.
			return true;
		} else if ( ! empty( \Vamtam_Elementor_Utils::get_theme_site_settings( "{$theme_setting_prefix}disable_all_wc_mods", $from_db ) ) ) {
			// All theme wc mods disabled by user pref.
			return false;
		} else {
			// User pref for current wc.
			return ! empty( \Vamtam_Elementor_Utils::get_theme_site_settings( $theme_setting_prefix . $wc_mod_name, $from_db ) );
		}
	}
